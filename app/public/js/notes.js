...
const NOTES_RU = {
    ...
    // ========== ДИЕЗЫ (♯) — ТА ЖЕ ПОЗИЦИЯ, ДРУГАЯ ЧАСТОТА ==========
...
function normalizeToOctave(detectedFreq, targetFreq) {
    ...
    // 👇 ПРОСТО ПРИВОДИМ К БЛИЖАЙШЕЙ ОКТАВЕ, НО НЕ ПРИВЯЗЫВАЕМСЯ К ТАРГЕТУ!
    // Просто чтобы частота была в диапазоне 100-800 Гц
    while (freq < 100) {
        freq = 100;
    }
    while (freq > 800) {
